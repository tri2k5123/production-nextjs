...
function Header() {
    ...
    const { openFormLogin, setOpenFormLogin, openFormRegister, setOpenFormRegister } = useContext(StateGlobalContext);
                ...
                <div className="flex items-center justify-between flex-1">
                    <div className="text-color-text text-xs flex items-center">
                        {listCategories.map((navItem, index) => (
                            <Link key={index} href={`https://ryanflo.vercel.app/collections/${navItem.category}`} className="my-0 mx-3 px-0 py-4 relative block text-color-text circle hover:before:block">{navItem.name}</Link>
                        ))}
                    </div>
                </div>
                                    ...
                                    <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                                        <div className="p-4">
                                            {session?.user?.role === "user" ? (
                                                <>
                                                    <Link href={"https://ryanflo.vercel.app/profile-user"} className="items-center group relative flex gap-x-6 rounded-lg hover:bg-gray-50">
                                                    ...
                                                    <Link href={"https://ryanflo.vercel.app/detail-history-order"} className="items-center group relative flex gap-x-6 rounded-lg hover:bg-gray-50">
                                                        <div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                                                ...
                                                <Link href={"https://ryanflo.vercel.app/admin/profile-admin"} className="items-center group relative flex gap-x-6 rounded-lg hover:bg-gray-50">
                                                    <div className="mt-1 flex h-11 w-11 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-white">
                                                        <ClipboardDocumentListIcon className="h-6 w-6 text-gray-600 group-hover:text-indigo-600" aria-hidden="true" />
                                                    </div>
                                                    <div>
